...
-- | is w a member of the dictionary
member :: ByteString -> ByteString -> Bool
member db w = 
  let p = findLT db w
      w' = takeWord db p
      len = BS.length w
      len' = BS.length w'
  in BS.isPrefixOf w w' && len' > len && BS.index w' len == ' '

lookup :: ByteString -> ByteString -> Maybe ByteString
lookup db w =
  let p = findLT db w
      w' = takeWord db p
      len = BS.length w
      len' = BS.length w'
  in if BS.isPrefixOf w w' && len' > len && BS.index w' len == ' '
       then Just $ BS.drop (len+1) w'
       else Nothing

lookupInt :: ByteString -> ByteString -> Int
lookupInt db w = maybe 0 id $ do w' <- lookup db w
                                 (v,_) <- BS.readInt w'
                                 return v
