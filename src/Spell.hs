...
type Dict = ( ByteString -> Bool, ByteString -> Int )

singles :: [ ByteString ]
singles = map BS.singleton ['a'..'z']

edits :: ByteString -> [ ByteString ]
edits w = deletes <> nubSort (transposes <> replaces) <> inserts
  where
    splits     = zip (BS.inits w) (BS.tails w)
    deletes    = [ a <> (BS.drop 1 b) | (a,b) <- splits, BS.length b > 0 ]
    transposes = [ a <> c <> (BS.drop 2 b) | (a,b) <- splits, BS.length b > 1,
                   let c = BS.pack [ BS.index b 1, BS.index b 0 ] ]
    replaces   = [ a <> c <> (BS.drop 1 b) | (a,b) <- splits, BS.length b > 1,
                    c <- singles ]
...
correct :: Dict -> ByteString -> ByteString
